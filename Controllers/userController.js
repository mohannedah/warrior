...
exports.subscripe = async (req, res) => {
  try {
    ...
    const subscription = await stripe.subscriptions.create({
      ...
      items: [
        {
          price: "price_1I6zndHut03REFrTLcPa3d9f",
        },
      ],
    ...
    (user.subscription.status = status),
      (user.subscription.client_secret =
        client_secret || user.subscription.client_secret),
      (user.subscription.id = subscriptionID || user.subscription.id),
      await user.save();
    res.status(200).json({ status, client_secret, user });
  } catch (error) {
    console.error(error.message);
    res.status(500).json({ msg: "Internal Server Error" });
  }
